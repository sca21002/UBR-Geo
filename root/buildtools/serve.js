...
log.info('ubrGeo', 'Parsing dependencies ...');
...
manager.on('ready', function() {
  ...
  server.listen(options.port, function() {
    log.info('ubrGeo', 'Listening on http://localhost:' +
        options.port + '/ (Ctrl+C to stop)');
  });
  server.on('error', function(err) {
    log.error('ubrGeo', 'Server failed to start: ' + err.message);
    process.exit(1);
  });
